...
//    	}
	...
	// 如果是从自定义查询界面过来,则无需再显示保存查询按钮
	if(false){
//	if(!userQuery){
		var queryNaneField = new Ext.form.TextField({
		
		});
		tbar.add('查询名称:',queryNaneField);
		var queryType = ComboboxExp.getCombobox({
				store:ComboboxExp.getComboboxStore({url:'servlet/userQuery',
						params:{type:'querySelfQueryType'}}),
				forceSelection:false,
				width    : 200
		});
		tbar.add(' ','查询分类:',queryType);
		tbar.add({text:'保存查询',iconCls: 'save_userquery',tooltip:'输入查询名称后,可以对本次查询进行保存!',handler:saveUserQuery});
	}
	//保存查询
  	if(!userQuery){
		var queryNaneField = new Ext.form.TextField({});
		tbar.add('查询名称:',queryNaneField);
		var queryType = ExtFormUtil.getCommonComboBox({
